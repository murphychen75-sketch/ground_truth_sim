...
bool usv_interfaces__msg__vision_detection__convert_from_py(PyObject * _pymsg, void * _ros_message)
{
  ...
  {  // pixel_width
    PyObject * field = PyObject_GetAttrString(_pymsg, "pixel_width");
    if (!field) {
      return false;
    }
    assert(PyFloat_Check(field));
    ros_message->pixel_width = PyFloat_AS_DOUBLE(field);
    Py_DECREF(field);
  }
  {  // confidence
...
PyObject * usv_interfaces__msg__vision_detection__convert_to_py(void * raw_ros_message)
{
  ...
  {  // pixel_width
    PyObject * field = NULL;
    field = PyFloat_FromDouble(ros_message->pixel_width);
    {
      int rc = PyObject_SetAttrString(_pymessage, "pixel_width", field);
      Py_DECREF(field);
      if (rc) {
        return NULL;
      }
